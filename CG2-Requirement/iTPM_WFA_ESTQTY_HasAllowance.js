...
function(search,record,runtime) {
	...
	function checkAllowanceForPromoDealWithItem(scriptContext) {
		try{
			...
			var promoDealId = scriptObj.getParameter({name:'custscript_itpm_estqty_hasallowance_item'}),
			estVolumeItemId = scriptObj.getParameter({name:'custscript_itpm_estqty_hasallownce_promo'});
