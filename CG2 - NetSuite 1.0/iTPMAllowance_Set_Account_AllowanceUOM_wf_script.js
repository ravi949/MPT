...
function setAccountAndAllowanceUOM() {
	try{
		...
		itemBasePrice = nlapiGetFieldValue('custrecord_itpm_all_itembaseprice');
		...
		case '1': //Rate per UOM calculate %Discount price
			...
			var percentBasePrice = (itemPrice !='' && itemPrice != 0) ? (allowanceRateValue/(conversionFactor*itemPrice))*100:0;
		...
		case '2': //%Discount calculate rate UOM
			...
			var ratePerUOM = percentBasePrice * conversionFactor * itemPrice * redemptionFactor;
